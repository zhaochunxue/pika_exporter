...
func (e *exporter) scrape(ch chan<- prometheus.Metric) {
	...
	for _, instance := range e.dis.GetInstances() {
		fut.Add()
		go func(addr, password, alias string, timeout int) {
			...
			c, err := newClient(addr, password, alias, timeout)
			if err != nil {
		...
		}(instance.Addr, instance.Password, instance.Alias, instance.Timeout)
	}
...
func (e *exporter) collectInfo(c *client, ch chan<- prometheus.Metric) error {
	info, err := mergeInfo(c)
	if err != nil {
		return err
	}
	// merge data stats if not nil
	if lastDataStats, exists := e.lastDataStats[c.addr]; exists {
		info += lastDataStats
	}
...
func mergeInfo(c *client) (string, error) {
	infoServer, err := c.InfoServer()
	if err != nil {
		return "", err
	}
	infoClients, err := c.InfoClients()
	if err != nil {
		return "", err
	}
	infoStats, err := c.InfoStats()
	if err != nil {
		return "", err
	}
	infoCpu, err := c.InfoCpu()
	if err != nil {
		return "", err
	}
	infoReplication, err := c.InfoReplication()
	if err != nil {
		return "", err
	}
	infoCmd, err := c.InfoCmd()
	if err != nil {
		return "", err
	}
	infoKeyspace, err := c.InfoKeyspace()
	if err != nil {
		return "", err
	}
	return infoServer + infoClients + infoStats + infoCpu + infoReplication + infoCmd + infoKeyspace, nil
...
func (e *exporter) statsData() {
	...
	for range ticker.C {
		for _, v := range e.dis.GetInstances() {
			c, err := newClient(v.Addr, v.Password, v.Alias, 50)
			if err != nil {
				log.Warnln("stats data new pika client failed. err:", err)
				continue
			}
			if infoData, err := c.InfoData(); err != nil {
				log.Warnln("stats data execute  failed. err:", err)
			} else {
				e.lastDataStats[v.Addr] = infoData
			}
			c.Close()
		}
	}
...
func (e *exporter) statsKeySpace(hour int) {
	...
	for {
		...
		for _, v := range e.dis.GetInstances() {
			c, err := newClient(v.Addr, v.Password, v.Alias, v.Timeout)
			if err != nil {
